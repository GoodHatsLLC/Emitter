...
actor AsyncValue<T: Sendable> {
  ...
  /// > Important: `T` will be resolved and suspended accesses resumed even if a passed `action`
  /// closure causes this function to throw.
  func resolve(to value: T, action: @escaping () throws -> Void = { }) rethrows {
    guard _value == nil
    else {
      return
    }
  ...
  /// > Important: `T` will be resolved and suspended accesses resumed even if a passed `action`
  /// closure causes this function to throw.
  func resolve(to value: T, action: @escaping () async throws -> Void) async rethrows {
    guard _value == nil
    else {
      return
    }
...
extension AsyncValue where T == Void {
  ...
  /// `action`
  /// closure causes this function to throw.
  func resolve(action: @escaping () async throws -> Void) async rethrows {
